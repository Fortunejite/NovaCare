...
export default function DoctorLayout({ children }: { children: React.ReactNode }) {
  ...
  const { doctor, fetchDoctor, status } = useDoctorStore();
  const pathname = usePathname();
  const router = useRouter();
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

  useEffect(() => {
    if (user && user.role === 'doctor' && !doctor && status === 'idle') {
      fetchDoctor();
    }
  }, [user, doctor, status, fetchDoctor]);
  ...
  const navItems = [
    ...
    { name: 'Consultations', href: '/doctor/consultations', icon: ClipboardPlus },
  ];
    ...
    <div className="min-h-screen bg-slate-50/50 dark:bg-zinc-950 flex flex-col md:flex-row text-foreground">
      <header className="md:hidden flex h-16 w-full items-center justify-between border-b border-border bg-card px-4 sm:px-6 sticky top-0 z-50">
        <Link href="/doctor" className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-semibold tracking-tight text-foreground">NovaCare Clinic</p>
      ...
      <aside className="hidden md:flex flex-col w-64 border-r border-border bg-card sticky top-0 h-screen p-5 shrink-0 z-30 justify-between">
        <div className="space-y-6">
          <Link href="/doctor" className="flex items-center gap-3 px-2">
            ...
            <div>
              <p className="text-sm font-bold tracking-tight text-foreground">NovaCare Clinic</p>
              <p className="text-[11px] text-muted-foreground font-medium">Doctor Workspace</p>
            </div>
          </Link>
          ...
          {doctor ? (
            <div className="p-3.5 rounded-2xl bg-slate-50/80 dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 space-y-2">
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-[10px] text-muted-foreground truncate">
                  {doctor.departmentName} · Lic: {doctor.licenseNumber}
                </p>
              </div>
            </div>
          ) : (
