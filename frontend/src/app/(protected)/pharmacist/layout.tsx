...
import { LayoutDashboard, Pill, ClipboardList, LogOut, Menu, X, ShieldCheck } from 'lucide-react';
...
export default function PharmacistLayout({
  ...
  const { pharmacist, fetchPharmacist, status } = usePharmacistStore();
  const pathname = usePathname();
  const router = useRouter();
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

  useEffect(() => {
    if (user && user.role === 'pharmacist' && !pharmacist && status === 'idle') {
      fetchPharmacist();
    }
  }, [user, pharmacist, status, fetchPharmacist]);
  ...
  const navItems = [
    ...
    { name: 'Medications', href: '/pharmacist/medications', icon: Pill },
    { name: 'Prescriptions', href: '/pharmacist/prescriptions', icon: ClipboardList },
  ];
    ...
    <div className="min-h-screen bg-slate-50/50 dark:bg-zinc-950 flex flex-col md:flex-row text-foreground">
      ...
      <header className="md:hidden flex h-16 w-full items-center justify-between border-b border-border bg-card px-4 sm:px-6 sticky top-0 z-50">
        <Link href="/pharmacist" className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-semibold tracking-tight text-foreground">NovaCare Pharmacy</p>
            <p className="text-[10px] text-muted-foreground">Pharmacist Portal</p>
          </div>
        </Link>
        <Button
          variant="ghost"
          ...
          <div className="border-t border-border pt-4 mt-auto">
            {pharmacist && (
              <div className="px-4 py-3 mb-4 rounded-xl bg-muted/40 border border-border flex flex-col gap-0.5">
                <p className="text-xs font-semibold text-foreground">{pharmacist.firstName} {pharmacist.lastName}</p>
                <p className="text-[10px] text-muted-foreground font-mono">Lic: {pharmacist.licenseNumber}</p>
              </div>
            )}
            <Button
              variant="destructive"
      ...
      <aside className="hidden md:flex flex-col w-64 border-r border-border bg-card sticky top-0 h-screen p-5 shrink-0 z-30 justify-between">
        <div className="space-y-6">
          <Link href="/pharmacist" className="flex items-center gap-3 px-2">
            ...
            <div>
              <p className="text-sm font-bold tracking-tight text-foreground">NovaCare Pharmacy</p>
              <p className="text-[11px] text-muted-foreground font-medium">Pharmacist Workspace</p>
            </div>
          </Link>
